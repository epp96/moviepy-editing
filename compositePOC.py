...
overlay_path = "movies/overlays/pexels-atypeek-dgn-7043616.mp4"
background_path = "pictures/pexels-francesco-ungaro-1448645.jpg"

overlay = VideoFileClip(overlay_path)
background = ImageClip(np.array(Image.open(background_path).resize(overlay.size)))

audio = loop_audio(videoDuration,  overlay.audio)

final = CompositeVideoClip([background, overlay], size=overlay.size).set_duration(videoDuration).set_audio(audio)
